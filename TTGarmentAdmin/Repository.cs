...
namespace TTGarmentAdmin
{
    ...
    {
        private string ApiUrl = "http://mps.bisplindia.in/api/home/";
        ...
        {
            var result = await CallGetFunction("SendOtpToUser/" + MobileNo);
            ...
            {
                var otpDetail = result;
                return otpDetail;
            }
        }

        public async Task<Response> GetPageDetail(int UserId, int PageId)
        {
            var result = await CallGetFunction("ListMediaByUserId/" + UserId + "/" + PageId);
            if (result == null || !result.Status)
            {
                return null;
            }
            ...
            {
                var pageDetail = result;
                return pageDetail;
            }
        }

        public async Task<List<MediaTypeList>> GetMediaList()
        {
            var result = await CallGetFunction("ListMediaTypeList");
            if (result == null || !result.Status)
            {
                return null;
            }
            ...
            {
                var otpDetail = JsonConvert.DeserializeObject<List<MediaTypeList>>(result.ResponseValue);
                return otpDetail;
            }
        }


        public async Task<UserDetail> AdminLogin(LoginViewModel loginModel)
       {         
            var loginDetail = "{\"mobileNo\":\"UsernameValue\",\"otp\":\"PasswordValue\"}";
            loginDetail = loginDetail.Replace("UsernameValue", loginModel.mobileNo).Replace("PasswordValue", loginModel.otp);

            var result = await CallPostFunction(loginDetail, "Login");
            if (result == null || !result.Status)
            {
                return null;
            }
            ...
            {
                var memberDetail = JsonConvert.DeserializeObject<UserDetail>(result.ResponseValue);
                return memberDetail;
            }
        }
